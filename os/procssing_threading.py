...
outpath = "/Users/maxuan/Desktop/1/os/test.csv"
...
if __name__ == '__main__':
    ...
    p1 = multiprocessing.Process(target=deal,args=(1,200))
    p2 = multiprocessing.Process(target=deal,args=(201,200))
    p3 = multiprocessing.Process(target=deal,args=(401,200))
    p1.start()
    p2.start()
    p3.start()
    deal(601,200)
    multi_process_end = time.time()
    print('\nMulti Process cost time:', multi_process_end - multi_process_start)
    multi_thread_start = time.time()
    t1 = threading.Thread(target=deal,args=(1,200))
    t2 = threading.Thread(target=deal,args=(201,200))
    t3 = threading.Thread(target=deal,args=(401,200))
    deal(601,200)
    multi_thread_end = time.time()
    print('\nMulti Thread cost time:', multi_thread_end - multi_thread_start)